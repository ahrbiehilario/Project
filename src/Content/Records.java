...
private void handleStatusClick(int row) {
    ...
    if (statusValue instanceof String) {
        ...
        if ("Fully Paid".equals(currentStatus) || "Canceled".equals(currentStatus)) {
            JOptionPane.showMessageDialog(this, "This reservation is already " + currentStatus + " and cannot be modified.", "Action Denied", JOptionPane.WARNING_MESSAGE);
            return;
        }
...
private void updateReservationStatus(int resID, String newStatus) {
    ...
    private void updateStaffHistory(int resID, String status, int fee) {
    try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/accounts", "root", "sha@123")) {
        StringBuilder sql = new StringBuilder("UPDATE staffhistory SET ");
        boolean needsComma = false;
        if (status != null) {
            sql.append("hisStatus = ?");
            needsComma = true;
        }
        if (fee != -1) {
        ...
        try (PreparedStatement pst = con.prepareStatement(sql.toString())) {
            int paramIndex = 1;
            if (status != null) pst.setString(paramIndex++, status);
            if (fee != -1) pst.setInt(paramIndex++, fee);
            pst.setInt(paramIndex, resID);
            pst.executeUpdate();
        }
    } catch (SQLException e) {
        JOptionPane.showMessageDialog(this, "Error updating staff history: " + e.getMessage(), "Database Error", JOptionPane.ERROR_MESSAGE);
    }
}
